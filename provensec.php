...
define('PROVENSEC_SERVICES', 'https://cloudprox.provensec.com/servers/') ;

define('PROVENSEC_VERSION', get_option('provensec_version'));

/*Register activation Hook start */

$new_version = '1.0.2';

if($new_version !== PROVENSEC_VERSION)
{ 
	update_option('provensec_version' , $new_version ) ;
	create_tables();
}
...
function provensec_activate() {
    ...
    'api_key' => '', 
    'assets_number' => '', 
    'user_asset_price_onetime' => '', 
    'user_asset_price_daily' => '', 
    'paypal_email' => '', 
    'paypal_logo' => '', 
    'enable_capthca' => '', 
    'ipn_page' => '');
...
function provensec_create_pages() {
    ...
    if (empty($is_page2) || get_post_status($is_page2) != 'publish') {
        $page_args = array('post_title' => __('Provensec payment cancel', 'provensec'), 'post_content' => '<div class="notice_error">' . __('You have cancelled your payment.', 'provensec') . '</div>', 'post_status' => 'publish', 
        'post_type' => 'page');
...
function initialize_plugin() {
    ...
    if (isset($_GET['provensec_do_payment']) && !empty($_GET['provensec_do_payment'])) {
        ...
        if (!empty($data)) {
            ...
            $paypal_query['custom'] = $userId;
            
            $paypal_query['return'] = site_url() . '?p_ipn_true=1';
            
            $paypal_query['notify_url'] = $ipn_url;
            
            $paypal_query['cancel_return'] = site_url() . '?p_ipn_error=1';
            ...
            $paypal_query['amount_1'] = $data->total_amount;
    ...
    if (isset($_GET['p_ipn_true']) && $_GET['p_ipn_true'] == 1) {
    ...
    if (isset($_GET['p_ipn_error']) && !empty($_GET['p_ipn_error'])) {
...
function signup_user() {
    ...
    if(empty($provensec_assets))
    {
        $is_valid = array();
        $is_valid[] = __('Please select at least one asset', 'provensec') ;
    }
    
    if ($asset_host < $provensec_assets) {
        $is_valid = array(); 
        $is_valid[] = __('Please add all the assets','provensec');
    }

    $assets = array();
    $one_time_asset_payment = 0;
    $daily_asset_payment = 0;
    $provensec_total_payment = 0;    
    if (!empty($asset_host)) {
        array_filter($asset_host);        
        foreach ($asset_host as $key => $value) {            
            $asset_nm = $asset_name[$key];            
            if (empty($asset_nm)) {
                $asset_nm = $value;
            }

            $single_asset = array('asset_name' => $asset_nm, 'asset_host' => $value, 'asset_type' => $asset_type[$key], 'subscription_type' => $subscription_type[$key]);

            $assets[] = $single_asset;

            if ($subscription_type[$key] == 'once') {
                $one_time_asset_payment = $one_time_asset_payment + $user_asset_price_onetime;
            } 
            elseif ($subscription_type[$key] == 'daily') {
                $daily_asset_payment = $daily_asset_payment + $user_asset_price_daily;
            }
        }
        $provensec_total_payment = $daily_asset_payment + $one_time_asset_payment;
    }

    if(empty($provensec_total_payment))
    {
        $is_valid = array() ; 
        $is_valid[] = __('Invalid payment value.','provensec') ;
    }
    ...
    if($is_valid === true) {
        ...
        if ($provensec_total_payment > 0) {
            $post_array['is_verification'] = '1';
        }
        ...
        if (empty($api_data)) {
            ...
            elseif ($api_statuss == 'ok') {
                if ($provensec_total_payment == 0) {
                    $success_page_id = get_option('provensec_success_page');
                    $success_page = get_permalink($success_page_id);
                    echo '<meta http-equiv="refresh" content="0;URL=' . $success_page . '">';
                    
                    //header('Location: '. $success_page) ; exit;
...
function provensec_shortcode() {
    ...
    */
    ...
    $return.= '<select name="provensec_country prov_select" id="provensec_country">';
    ...
    $return.= '<input type="hidden" name="user_asset_price_daily" id="user_asset_price_daily" value="' . $user_asset_price_daily . '" />';
    $return.= '<input type="hidden" name="user_asset_price_onetime" id="user_asset_price_onetime" value="' . $user_asset_price_onetime . '" />';
    $return.= '<div class="provensec_select"><select name="provensec_assets" id="provensec_assets" class="prov_select"><option value="0">Select asset</option>';
    ...
    $return.= '</select></div>';    
    $return.= '<br><div><i><a id="add_asset" href="#">Click here</a> to add the target URL / IP that you would like to Scan. You can also skip this step , if you do not know the target information and would like to add the targets directly in the scanning portal.</i></div>';
    
    //$return .= '<a id="add_asset" href="#">Add Asset</a>' ;
    
    $return.= '<div id="assets_section">';
    
    if (isset($_POST['asset_name'])) {
        foreach ($_POST['asset_name'] as $key => $value) {
            
            $return.= '<div class="asset_row"><input type="text" name="asset_name[]" placeholder="Asset name" required value="' . $_POST['asset_name'][$key] . '"/><input type="text" name="asset_host[]" placeholder="Asset IP" value="' . $_POST['asset_host'][$key] . '" required/><select name="asset_type[]" id="asset_type[]" class="prov_select" required><option>Select Asset Type</option><option value="1" ' . checkSelected(1, $_POST['asset_type'][$key]) . '>Infrastructure device IP</option><option value="2" ' . checkSelected(2, $_POST['asset_type'][$key]) . '>HTTPS enabled server</option><option value="3" ' . checkSelected(3, $_POST['asset_type'][$key]) . '>HTTP only server</option></select><select name="subscription_type[]" id="subscription_type[]" class="scan_type prov_select" required><option value="">Select Scan Type</option><option value="once" ' . checkSelected('once', $_POST['subscription_type'][$key]) . '>Once</option><option value="daily" ' . checkSelected('daily', $_POST['subscription_type'][$key]) . '>Daily</option></select><span class="del_asset_row">×</span></div>';
        }
    }
    ...
    if ($enable_capthca == 'on') {
        ...
        $return.= '<div id="imgdiv"><img id="img" src="' . plugins_url('/captcha/captcha.php', __FILE__) . '" /><img id="reload" src="' . plugins_url('/captcha/reload.png', __FILE__) . '" /></div>';
...
function checkSelected($value, $postval) {
...
function provensec_setting_page() {
    ...
    if (isset($_POST['provensec_savesetting'])) {
        ...
        'user_asset_price_daily' => $user_asset_price_daily, 
        'user_asset_price_onetime' => $user_asset_price_onetime, 
...
?>
  ...
  <form name="provensec_setting" id="provensec_setting" action="" method="post">
    ...
    <label for="provensec_api" class="clear provensec_label">
      ...
      <?php
    _e('API Key', 'provensec'); ?>

    </label>

    <input type="text" name="provensec_api" id="provensec_api" class="prov_textbox" value="<?php
    echo $api_key; ?>"/>

    <span class="helptext">

    <?php
    _e('Enter 25 characters API key provided by provensec', 'provensec'); ?>
    ...
    <label for="provensec_user_name" class="clear provensec_label">
      ...
      <?php
    _e('Username', 'provensec'); ?>

    </label>

    <input type="text" name="provensec_user_name" id="provensec_user_name" class="prov_textbox" value="<?php
    echo $user_name; ?>"/>

    <span class="helptext">

    <?php
    _e('Enter the email id provided for above API key', 'provensec'); ?>
    ...
    <label for="user_assets" class="clear provensec_label">
      ...
      <?php
    _e('Number of assets', 'provensec'); ?>

    </label>

    <input type="text" name="user_assets" id="user_assets" class="prov_textbox" value="<?php
    echo $assets_number; ?>"/>

    <span class="helptext">

    <?php
    _e('Add max number of assets you want to show in drop down on registration form', 'provensec'); ?>
    ...
    <label for="user_asset_price" class="clear provensec_label">
      ...
      <?php
    _e('Price per asset ($) Daily scan', 'provensec'); ?>

    </label>

    <input type="text" name="user_asset_price_daily" id="user_asset_price_daily" class="prov_textbox" value="<?php
    echo $user_asset_price_daily; ?>"/>
    <span class="helptext">
    <?php
    _e('Add price per asset you want to charge for daily scan', 'provensec'); ?>
    </span>
    <label for="user_asset_price" class="clear provensec_label">
      <?php
    _e('Price per asset ($) One time scan', 'provensec'); ?>
    </label>

    <input type="text" name="user_asset_price_onetime" id="user_asset_price_onetime" class="prov_textbox" value="<?php
    echo $user_asset_price_onetime; ?>"/>
    <span class="helptext">
    <?php
    _e('Add price per asset you want to charge for one time scan', 'provensec'); ?>
    </span>
    <label for="provensec_paypal" class="clear provensec_label">
      <?php
    _e('Paypal email', 'provensec'); ?>
    </label>

    <input type="text" name="provensec_paypal" id="provensec_paypal" class="prov_textbox" value="<?php
    echo $paypal_email; ?>"/>

    <span class="helptext">

    <?php
    _e('Enter business paypal account email address', 'provensec'); ?>
    ...
    <label class="clear provensec_label">
      ...
      <?php
    _e('Upload logo for paypal', 'provensec'); ?>

    </label>

    <p>

      <input type="hidden" class="my-media-uploader-input" id="paypal_logo" name="paypal_logo" value="<?php
    if (!empty($paypal_logo)) {
        echo $paypal_logo;
    } ?>"/>
      ...
      <img class="<?php
    echo $logo_class; ?>" src="<?php
    if (getimagesize($paypal_logo) !== false) {
        echo $paypal_logo;
    } ?>" id="logo_img"/> </p>

    <div style="clear:both;height:20px;"></div>

    <input type="checkbox" name="enable_captcha" id="enable_captcha"  <?php
    if ($enable_capthca == 'on') {
        echo 'checked';
    } ?>/>
    ...
    <label for="provensec_success_page" class="provensec_label">
      ...
      <?php
    _e('Payment success page', 'provensec'); ?>
    ...
    if (!empty($pages)) {
        ...
        echo '<select name="provensec_success_page prov_select" id="provensec_success_page">';
...
?>
    ...
    <?php
    _e('Select the page to show redirect after successfull payment', 'provensec'); ?>
    ...
    <p>
      ...
      <input class="btn" type="submit" name="provensec_savesetting" id="provensec_savesetting" value="<?php
    _e('Save', 'provensec'); ?>" />
...
?>
    ...
    echo get_permalink($signup_page_id); ?>">

      <?php
    _e('Click here to view the registration page', 'provensec'); ?>

      </a></h3>

    <?php
    _e('You can also show registration form on any page or post using below shortcode :', 'provensec'); ?>
